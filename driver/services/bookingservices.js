...
exports.bookingComplete = (driverid)=>
{
    return new Promise((resolve, reject)=>{
        promise.coroutine(function * (){
            ...
            let res=dbHandler.dbHandlerPromise(query2)
            let totalrating=res[0].total_rating+rating[0].rating;
            let totaltrips =res[0].no_of_trips+1;
            let query=`UPDATE driver d,booking b SET d.status=0,b.bookingstatus=0,d.total_rating=${totalrating},d.no_of_trips=${totaltrips} where d.id=${driverid} AND b.driverid=${driverid} AND d.status=1`;
